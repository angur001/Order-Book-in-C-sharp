...
// Written entirely against IPriceLadder rather than SortedDictionaryPriceLadder's
// own members (beyond construction), so this class - or a copy of it - can be
// pointed at a future second implementation (e.g. a tick-indexed array/ring-buffer
// ladder) to verify it's behaviorally interchangeable with this one.
public class TreePriceLadderTests
{
    private static TreePriceLadder CreateAscendingLadder() =>
        new(Comparer<Price>.Create((x, y) => x.Value.CompareTo(y.Value)));

    private static TreePriceLadder CreateDescendingLadder() =>
        new(Comparer<Price>.Create((x, y) => y.Value.CompareTo(x.Value)));
    ...
    // The riskiest part of this implementation: WorstPrice is a cache, not a
    // live computation, so removing the level that happens to be the current
    // worst must recompute it from what's actually left - not leave it
    // pointing at a price that no longer has a level at all.
    [Fact]
